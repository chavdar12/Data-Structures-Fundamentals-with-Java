...
public class Tree<E> implements AbstractTree<E> {
    ...
    private void fillTree(List<Tree<E>> children, StringBuilder fullTree, int spaces) {
    ...
    public List<E> getLeafKeys() {
        ...
        getLeaves(this.children, leaves);
        return leaves;
    }

    private void getLeaves(List<Tree<E>> children, List<E> leaves) {
        for (Tree<E> child : children) {
            if (child.children.size() == 0) {
                leaves.add(child.value);
            }
            getLeaves(child.children, leaves);
        }
    }
